...
function openChildren(event) {
    ...
    const vise = "vise";
    if (num%2===0) {
        document.getElementById(toString(category.id)+"1").className = vise;
        num++
    }
    else
    {
    document.getElementById(toString(category.id)+"1").className = viseOff;
    const tar=event.target;
    // tar.innerText = "down";
    num++
    }
    ...
    return (
        ...
        <ul className="category">
            ...
            <div id={toString(category.id)+"1"} className="vise">
            {
